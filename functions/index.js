...
// Tek-seferlik yardımcı: bir quoteNo'ya bağlı tüm feasibility study'lerin
// linkedQuoteNo + convertedAt alanlarını temizler. Kayıp teklif senaryosunda
// study'leri tekrar "approved" durumuna çevirmek için.
// GET /unlinkFeasibilityByQuoteNoHttp?quoteNo=26072201&year=2026
exports.unlinkFeasibilityByQuoteNoHttp = onRequest(
  {
    region: REGION,
    timeoutSeconds: 60,
    memory: "256MiB",
  ...
  async (req, res) => {
    const quoteNo = String(req.query.quoteNo || "").trim();
    const year = String(req.query.year || "2026").trim();
    if (!quoteNo) {
      res.status(400).json({ error: "quoteNo zorunlu (?quoteNo=26072201&year=2026)" });
      return;
    }
    try {
      const ref = db.collection("appData").doc(`feasibilityStudies_${year}`);
      const snap = await ref.get();
      if (!snap.exists) {
        res.status(404).json({ error: `feasibilityStudies_${year} doc bulunamadı` });
        return;
      }
      ...
      const update = {};
      const unlinked = [];
      for (const [studyNo, s] of Object.entries(studies)) {
        if (s && s.linkedQuoteNo === quoteNo) {
          update[`studies.${studyNo}.linkedQuoteNo`] = FV.delete();
          update[`studies.${studyNo}.convertedAt`] = FV.delete();
          update[`studies.${studyNo}.updatedAt`] = new Date().toISOString();
          unlinked.push(studyNo);
        }
      }
      if (unlinked.length === 0) {
        res.json({ success: true, unlinkedCount: 0, message: `${quoteNo} numarasına bağlı study bulunamadı` });
        return;
      }
      await ref.update(update);
      logger.info("[UnlinkFeasibility] ✓", { quoteNo, count: unlinked.length });
      res.json({ success: true, unlinkedCount: unlinked.length, studies: unlinked });
    } catch (err) {
      logger.error("[UnlinkFeasibility] Hata", { error: err.message });
      res.status(500).json({ error: err.message });
    }
  },
);
